async function enviarScript(scriptText){
	...
	for(const line of lines){
		...
		setTimeout(() => {
			(main.querySelector(`[data-testid="send"]`) || main.querySelector(`[data-icon="send"]`) || main.querySelector(`[data-icon="wds-ic-send-filled"]`)).click();
		}, 100);
		
		if(lines.indexOf(line) !== lines.length - 1) await new Promise(resolve => setTimeout(resolve, 350));
	}
